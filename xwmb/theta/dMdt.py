...
def calc_dMdt_theta(wmb, ds, snap, grid_snap, ocean_grid, mask, theta_i_bins, rho0=1035.):
    ...
    M_on_temp = (m_on_temp*ocean_grid.areacello).where(mask).sum(['xh', 'yh'])
    ...
    if np.all(np.isclose(dMdt['thetao_i'], wmb['thetao_i'], rtol=1.e-14)):
        wmb["dMdt"] = dMdt.assign_coords({'time':ds.time, 'thetao_i':wmb.thetao_i})
    else:
        raise ValueError("thetao_i coordinate data do not match.")
