...
def resolve_target_coords(grid, lambda_var, lambda_name=None, default_bins=False):
    ...
    The target coordinates may be supplied by the caller (already present in
    ``grid._ds``), derived from the pre-existing lambda coordinates, or generated
    from defaults (``default_bins=True``).
    """
    target_coords = {
    ...
    if not all(avail_lambda):
        raise ValueError(
            f"To specify the target grid, either pass `default_bins=True` or include "
            f"{target_coords['center']} and {target_coords['outer']} in "
            f"`WaterMassBudget.grid._ds`."
        )
...
def add_default_gridcoords(grid, lambda_var, lambda_name):
    ...
    if lambda_name is not None and "sigma" in lambda_name:
        lam_min, lam_max, dlam = 0.0, 50.0, 0.1
    elif lambda_name == "heat":
        lam_min, lam_max, dlam = -4.0, 40.0, 0.1
    elif lambda_name == "salt":
        lam_min, lam_max, dlam = -1.0, 50.0, 0.1
    else:
        raise ValueError(
    ...
    grid._ds = grid._ds.assign_coords(
        {
            ...
            f"{lambda_var}_i_target": np.arange(
                lam_min - dlam / 2.0, lam_max + dlam / 2.0, dlam
            ),
        }
    )
